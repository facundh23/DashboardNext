...
interface Props {
    // Siempre el tipo es string lo que nos viene por params
    params: { id: string };
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    try {
        ...
        return {
            title: `#${id} -  ${name}`,
            description: `age of pokemon ${name}`
        }
...
const getPokemonById = async (id: string): Promise<Pokemon> => {
    ...
    try {
        const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${id}`, {
            ...
            // Hacer que se revalide cada cierto tiempo la información de la api y actualizar los datos en la pagina
            // En este ejemplo el dato se renueva cada seis meses
            // next: {
